...
| Stage gestures | `editor_page.canvas.dart:171` | **Reuse the shape**: raw `Listener` + finger-count disambiguation (`_touchPos`/`_drawPointer`/`_pinching`), not gesture-arena recognizers. The Animator's cascade is *shorter* than the editor's (drag Actor / drag pivot / drag path handle / pinch-zoom vs. ~10 draft branches). Auto-key hooks into the same pointer-up commit point the editor uses to end strokes. *(Gesture grammar revised 2026-07-31 — select-then-drag-anywhere, a Move/Rotate mode, two-finger = view only: [06-gesture-safety.md](06-gesture-safety.md).)* |
