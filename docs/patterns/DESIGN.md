...
| On/Off UX | The page lists **Off** first; **long-press** (right-click on desktop) on the swatch toggles Off ↔ last pattern, and while On a tap on the green chip area around the tile turns it Off in one tap (2026-09-06; the tile itself still opens the page) without opening the page. Swatch: dimmed with a slash when Off; the tile in the primary color when On |
