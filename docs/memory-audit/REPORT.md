...
| 14 ◑ | Gallery: lazy per-viewport thumb decode + persistent thumb files (open); bound `PublishDraft`: **drop `docBytes` eagerly after the encode ✅** | Frees the ~document-sized snapshot across the metadata decode + page push | Tiny | **Low.** Review killed the other two ideas: `TransferableTypedData` copies anyway (no benefit); lazy `mkpxBytes` fights the build-time size gate. The dominant transient (`bytes`, the exported WebP that *is* the upload) is inherent |
