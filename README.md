...
A native (Android + iOS) **animated pixel-art editor** — smartphone-first, tablet-responsive — built on a
deterministic, headless **Rust engine** with a **Flutter** shell. The engine is the product's spine; the
visual app is a thin, fully-inspectable projection of its state, which keeps the
**develop → test → inspect → develop** loop fast and mostly screenshot-free.

## Documents
- **[`SPEC.md`](SPEC.md)** — the comprehensive specification: architecture, data model, memory/tiling/COW,
  the action-script DSL + verification harness, color/compositing, the `.mkpx` format, every tool/feature,
  undo + compaction, FFI, the three-row UI, Club-upload interface, performance budgets, and all resolved
  decisions.
- **[`PLAN.md`](PLAN.md)** — the development plan: Windows 11 environment setup, repo scaffold, the phased
...
- **MVP = layered, animated editor with core tools.** Dev/test on **Windows** (engine + desktop harness +
  Android emulator); **iOS deferred** to cloud macOS CI.
...
## Status — implemented & runnable (2026-06-25)

A working Rust engine + `mkpx` harness + FFI + **Flutter Windows app** are built and tested on this
workstation. See [`STATUS.md`](STATUS.md) for the full feature/coverage matrix.
...
- **App** (`app/`): the three-row UI (tool options · palette · tools) + canvas + timeline (reorder, per-frame
  duration) + layers (opacity/lock/reorder/duplicate-to-frames/multi-select group move) + multi-palette
  manager (`.gpl`/JSON) + RGB/HSV picker + import dialog + export + **Makapix Club upload**, over the engine
...
```
The prebuilt release app is at `app/build/windows/x64/runner/Release/makapix_editor.exe`.
