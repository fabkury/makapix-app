...
| **Replay & Timelapse** (☰ → Watch replay · Share timelapse) | ✅ | 1.2.0 (2026-08, ADRs 0003/0004): always-on per-drawing Journal (plain-text DSL lines with `+ms` deltas, chapter bases beside it, crash-torn-line repair) → a scrubbable in-app Replay over its own engine with COW checkpoints → a silent H.264 MP4 Timelapse (MediaCodec/VideoToolbox; WebP/GIF on Windows) with the finished piece as the finale. **Pacing since 2026-09-06 (ADR 0029, `docs/replay/PACING.md`):** the viewer and the export share one video-time axis paced by **clamped working time** — every visible tick dwells in proportion to the recorded time it took (gaps clamped at 2 s, invisible tuning/fiddling time flowing into its apply/commit), event ticks (fills, applies, commits, structural edits, undo, frame hops) clamped into a floor (max(6 frames, T/100)) and a ceiling (T/12), repeats of one event verb within 300 ms flowing as a burst; a journal with no timing paces uniformly. Shell-only; every existing journal gets it; scrubbing previews the export frame for frame; Pixel light pass OK 2026-09-06 |
...
| Feature | Status | Notes |
|---|---|---|
| Up to 1024 frames / 128 layers | ✅ | enforced caps (layers 64 → 128 on 2026-09-14, ADR 0032) |
...
| **Patterns** (dither / texture masks) | ✅ | 2026-09-04 (ADR 0025, `docs/patterns/DESIGN.md`): a canvas-anchored bitmask **gate** on Pencil/Brush/Eraser/Bucket — ON cells paint, OFF cells are untouched, one global tile with per-tool On/Off, a "Pattern" swatch at the end of row-1 (tap = the Patterns page: Off, 10 persisted recents, a 116-tile catalog of Bayer ladders with inverses, lines, diagonals/crosshatch, dots/grids/bricks/checkers; long-press = Off ↔ last; while On, a tap on the green edge around the tile = Off in one tap), AA inert while on, pixel-perfect thins first, the Bucket's region unchanged and its Repeat snapshotting the tile. The **Gradient** gets its own "Dither" swatch: ordered dithering between adjacent stops (exactly two stop colors per pixel, alpha included, smoothstep honored) — Bayer 2/4/8 since ADR 0025, and since 2026-09-06 (ADR 0028) **blue noise** (a shipped 64×64 void-and-cluster table), **halftone** 4/8 (clustered dots), **lines** (horizontal/vertical 2/4/8, diagonal 4/8), **white noise** and **interleaved gradient noise** (integer-exact), listed by section on the Dither page with 50 %-density previews. Journal verbs `SetPattern(w,h,hex)` / `SetPattern(off)` / `SetGradientDither(token)` carry the bits (tiles up to 16×16 on the wire; Bayer keeps its bare numbers, the other families are words); no `.mkpx`/FFI change |
...
| Duplicate / reorder animation frame | ✅ | film-roll of frame previews at the top of the canvas (tap to go to a frame; long-press for duplicate/duration/move/delete and the Frames page); engine-rendered cached thumbnails; **multi-frame operations live on the Frames page** (next row). **Delete frame / Delete layer confirm by a second tap** (ADR 0022, 2026-09-03): the sheet button arms on the first tap ("Tap again to confirm", filled red) and deletes on a second tap within 3 s; the window's expiry, a retarget, any other sheet action, or closing the sheet disarms it silently; the keyboard Delete frame command arms the same way (toast) |
| Per-frame duration 16.6–1000 ms + bulk tools | ✅ | µs-precise; UI dialog (this frame / all frames / fps presets) — the field/slider/chips are the shared `DurationEditor` since 2026-09-11, also used by the Frames page |
| **Frames page** (batch frame operations, ADR 0031) | ✅ | **Implemented 2026-09-11, polished 2026-09-14, unreleased** (`docs/frames-page/DESIGN.md`). The frame sheet's "Frames…" · ☰ → Frames… · Shift+T open a full-screen grid of every frame (3–8 columns by pinch or menu, persisted; point-sampled engine thumbnails cached by frame id). Tap toggles, a sideways slide sweeps (the first tile's new state paints onto every tile crossed; edge auto-scroll; an up/down slide scrolls; a second finger cancels a sweep and restores the selection, so a pinch never selects), double-tap goes to the frame, long-press / right-click open a tile menu (Go to · Select to here · Frame options); selection is amber, the active marker the strip's blue; helpers All · None · Invert · Select frames… (1-based ranges) · Every Nth; desktop Shift-click, Ctrl-click, rubber-band, Ctrl+A, Esc, Delete, ←/→, Ctrl+Z/Y, Enter. Every operation is **one engine verb on a frame set** (`RemoveFrames(12-32 40)`), one undo step, all-or-nothing: Delete (tap-again, ADR 0022) · Duplicate (copies become the selection) · Repeat after (block) · Insert blank before/after · rigid Shift (clamped; the bar's ‹ › pair hold-to-repeats) · Reverse · Set duration (More → Timing: one value / fps presets; pinned frames reported) · Scale × · Flip H/V · Rotate 90/180/270 (byte-identical to the single verb) · Invert · Copy active layer to frames (strict) · Remove / Show / Hide / Lock / Unlock layer named… (topmost hit per frame, hit counts shown; an emptied frame gets a blank layer). A set past the roll, the 1024-frame or 128-layer cap, or a delete that would empty the roll refuses with the reason on the page's status line (`refusal_seq` / `last_refusal` telemetry). The active target moves only when its frame is removed; the mask is never touched; undo/redo work in place. `DocStructure` undo records are now billed by the tiles they retain (a content batch over many frames evicts properly; the More sheet warns above 64 MB, never refuses). Windows + Pixel passes pending |
...
| Import GIF/WebP/PNG/APNG/JPEG/BMP (crop/scale/**1:1**, start-frame, as-layer, **off-canvas parking** — ADR 0030, 2026-09-09) | ✅ | **Open** (File → Open, 2026-09-03) accepts the same raster formats as a **new drawing at native size** — one frame per animation frame with its duration, title from the file name, provenance marked imported; refused above 512 per side with a pointer to Import (CONTEXT.md: Open preserves or refuses, Import converts); the picked bytes are sniffed for the `.mkpx` signature, never trusted by extension, so mislabeled files still open. **Import:** all formats; import options dialog; **dedicated crop editor** — static + animated preview (play/pause), draggable corner reticles, X/Y/W/H numeric entry, optional canvas-aspect lock, **zoomable/pannable view** (2026-09-01: one finger edits the crop, two fingers/trackpad pan + pinch, wheel zooms at the cursor, right/middle-drag pans, double-tap fit ↔ 4×, fit … 32 px/px); the region is placed **1:1 centered** (downscaled to fit only when larger than the canvas, never upscaled). The Fit/Stretch/Crop chooser appears only for a source larger than the canvas (Crop opens the editor at once), plus a **1:1** choice — preselected — when the source also fits within the 3×3 storage (ADR 0030: the whole source at its own size, overhang parked off-canvas); a smaller source is placed 1:1 centered with an optional "Scale up to fit" switch, an exact-size source has no scaling UI. **Place step** (ADR 0019, 2026-09-01): whenever the result is not exactly the canvas, the dialog's Next opens a pre-import page — canvas + dimmed gutter with the start frame's storage-sized composite as backdrop (fit frames the canvas; pan/zoom-out reach the gutter, floor = storage-fit), the animated import floating, one-finger drag snapped to canvas px, X/Y chips + ±1 px arrows, anywhere in storage allowed — and its Import commits with an engine `placement` offset. **Off-canvas parking** (ADR 0030, 2026-09-09): the import rasterizes into the storage-sized layer buffer, so the part outside the canvas is *parked* in the gutter (Move / Overscan view reach it; export stays canvas-cropped) and only the part beyond storage is dropped (shaded; Import refused only when nothing lands); a fixed-height status block (no reflow) shows parked/dropped px and an upper-bound memory estimate (frames × kept area × 4 B) that turns amber when the import would cross the hard budget — warning only, no publish-time gutter strip by decision; Crop and Place share one decoded preview |
| Export PNG / sprite-sheet / GIF | ✅ | PNG + animated GIF wired in UI; sprite-sheet in codec |
| Canvas ops: invert, resize, crop-to-selection, **Crop canvas** (2026-09-06, ADR 0027), **rotate canvas 90/180/270, flip canvas H/V** | ✅ | rotate/flip-canvas in the timeline ☰ menu's grouped **Canvas** submenu; resize/crop/rotate undoable (canvas size travels with the edit). **Crop canvas…** (Canvas submenu) opens the import flow's first-class crop editor over the document's composited frames (checker, play/step, zoom, reticles, X/Y/W/H, aspect lock, 16²…512² presets, Club-size note), seeded by the selection's bounds when one exists; **Trim to content** sets the tight box around all non-transparent pixels (all frames + layers); Crop issues one `CropCanvas(x,y,w,h)` verb (one undo step, replay-visible); disabled on the whole canvas |
...
| **C4** Artist dashboard (aggregate + per-post) | ✅ | `ui/artist_dashboard_page.dart`; totals + country/device/emoji breakdowns + per-post table + authenticated-only toggle (§19). Per-post `/post/{id}/stats` drill-in shipped 2026-07-29: owner ⋮ → **Statistics** (`ui/post_stats_page.dart` — 30-day daily-views trend, country/device/type/emoji breakdowns, force-refresh). **Device attribution (2026-09-11, unreleased):** every Club request carries `User-Agent: MakapixClub/<version>+<build> (<platform> <os>; <model>)` (`api/club_user_agent.dart`, resolved once at startup) so the server's device buckets split `app_android` / `app_ios` instead of the platform-less `app` it inferred from the Dart default UA; view registration also sends `intent: "view"` explicitly (server thread `messages/0001-app-device-type/`) |
...
4. ~~**Trim to non-transparent bounds** (§28.1)~~ — **✅ 2026-09-06**: the Crop canvas page's **Trim to content** button (ADR 0027).
