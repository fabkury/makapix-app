...
extension _EditorToolgrid on _EditorPageState {
  ...
  // The shared control idiom lives in lib/ui/chrome_controls.dart (lifted 2026-07-30 so the
  // Animator shares it). These four forwarders keep row-1's ~30 call sites context-free.
  void _labeledSlider(List<Widget> children, String name, double value, double min, double max,
      ...
      labeledSlider(context, children, name, value, min, max, onChanged,
          integer: integer, decimals: decimals);

  void _labeledLogSlider(List<Widget> children, String name, double value, double min,
          double max, ValueChanged<double> onChanged) =>
      labeledLogSlider(context, children, name, value, min, max, onChanged);

  void _labeledPowSlider(List<Widget> children, String name, double value, double min,
          double max, ValueChanged<double> onChanged, {bool integer = true}) =>
      labeledPowSlider(context, children, name, value, min, max, onChanged, integer: integer);

  Future<void> _editSliderValue(String name, double value, double min, double max,
          ValueChanged<double> onChanged, {required bool integer, int decimals = 1}) =>
      editSliderValue(context, name, value, min, max, onChanged,
          integer: integer, decimals: decimals);
