...
[`docs/BUILDING.md`](../docs/BUILDING.md), [`CLAUDE.md`](../CLAUDE.md), [`SPEC.md`](../SPEC.md)
(editor engine), and [`SPEC-CLUB.md`](../SPEC-CLUB.md) (social layer) for the full picture.

## Structure (`lib/`)

- `main.dart` — thin Flutter entry point.
- `app.dart` — the neutral root `MaterialApp`.
- `shell/` — `app_shell.dart`, the two-pillar shell (Club + Editor) mounting **one pillar at a
  time**; opens on Club, with the editor one ⊕ tap away (no login required).
